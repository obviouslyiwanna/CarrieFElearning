...
type Article = { title: string; summary: string; category: string; date: string; readingTime: string; accent: string; status: "已发布" | "学习中" };

const articles: Article[] = [
  { title: "useEffect 实战：等待依赖准备好再发起一次查询", summary: "从一个 ProTable 列表页的真实场景出发，拆开理解副作用、依赖数组与异步枚举之间的执行时机。", category: "React 基础", date: "2026.08.06", readingTime: "6 min", accent: "orange", status: "已发布" },
  { title: "把复杂需求拆成可维护的 React 页面", summary: "配方管理、扣款单、采退订单：从业务流程到组件边界，记录我如何把一次交付变成可复用的解题路径。", category: "工程实践", date: "即将开始", readingTime: "—", accent: "blue", status: "学习中" },
  { title: "React 学习路线：从渲染到状态管理", summary: "一张正在迭代的学习地图：先建立 mental model，再逐步补齐组件、Hooks、数据流和性能优化。", category: "学习路线", date: "即将开始", readingTime: "—", accent: "green", status: "学习中" },
];
  ...
  return (
    <main className="site-shell">
      ...
      <section className="section articles-section" id="articles">
        ...
        <div className="article-grid">{visibleArticles.map((article, index) => <article className={`article-card ${index === 0 ? "featured" : ""}`} key={article.title}><div className={`article-art ${article.accent}`}><span>{String(index + 1).padStart(2, "0")}</span><i /></div><div className="article-content"><div className="article-meta"><span className="tag">{article.category}</span><span>{article.status}</span></div><h3><a href={index === 0 ? "/articles/use-effect" : "#roadmap"}>{article.title}</a></h3><p>{article.summary}</p><div className="article-footer"><span>{article.date}</span><span>{article.readingTime} <b>→</b></span></div></div></article>)}</div>
