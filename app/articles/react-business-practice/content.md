...
第一个月接触了三个需求，分别是**配方管理**、**采退订单**、**扣款单管理**，将其放在一起观察，虽然它们虽然业务需求不同，但前端通常都要面对相似的问题：列表查询、条件筛选、详情和编辑、异步枚举、动态表格、金额或数量计算、状态流转、附件与提交反馈。
...
| 操作区 | 保存、提交、有效、无效是否互斥？按钮权限和状态如何判断？ |
...
配方详情页的实际代码里，有多种状态：步骤、供应商行、附件、当前状态、保存中、提交中，以及用于触发子组件校验的 `detailValidateTick`。
...
不要把第三类状态和业务数据混在一起，也不要同时维护互相容易冲突的 `isBusy`、`saving`、`submitting`。状态越多，越要思考：这个值是源数据，还是可以从已有状态直接算出来？
...
这是页面状态机的一部分。扣款金额、采退数量这种重要数据尤其不能允许用户在重复点击后产生两次提交。
...
## 五、自定义hook ：`useUrlParams`：把 URL 当成页面输入
