...
这份档案不是把两个页面逐行翻译一遍，而是回到真实业务，回答四个问题：**为什么要做、最终做了什么、代码为什么这样组织、下一次还能复用哪些经验。**

本次主角是采购工作台 `purchase-app` 中的两条 H5 详情链路：

- `pages/package/demand-order/info`：要货单详情；
- `pages/package/shipment-order/info`：发货单详情。
...
> 本文以 2026-09-11 当前分支源码和该功能的 Git 演进记录为准。当前实现已经删除要货/发货详情 mock，文章不会把旧 mock 当成生产能力。源码静态链路已复核，但本文没有代替真实账号、真实接口和真机 H5 联调。
